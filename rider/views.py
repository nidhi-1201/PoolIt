...
def statusUpdate(request):
	...
	# gmaps = googlemaps.Client(key='AIzaSyCI52MKQL5Hso6BstBiiw2g7x6stevMrRM')
	...
	if rideDetils.status :
		if rideDetils.complete:
			return JsonResponse({'success':True,'driverId':rideDetils.driverId,'complete':True,'cost':0, 'expectedTime':rideDetils.expectedTime})
		else:
			return JsonResponse({'success':True,'driverId':rideDetils.driverId,'complete':False,'cost':0, 'expectedTime':rideDetils.expectedTime})
...
def rideHistory(request):
	rideDetails = get_object_or_404(ride, pk=request.user.username)
	return render(request, 'rideHistory.html', {'r': rideDetails})

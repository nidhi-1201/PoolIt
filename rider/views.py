...
def statusUpdate(request):
    ...
    my_dist_1 = gmaps.distance_matrix(rideDetails.pickUp , rideDetails.destination)['rows'][0]['elements'][0]["distance"]["value"]
    my_dist_1 = my_dist_1/1000.0
    my_dist_1 = int(my_dist_1 *10)
    print("hello ----------------------------------",id)
    if rideDetails.status :
        if rideDetails.complete:
            return JsonResponse({'success':True,'driverId':rideDetails.driverId,'complete':True,'cost':my_dist_1, 'expectedTime':rideDetails.expectedTime})
        else:
            return JsonResponse({'success':True,'driverId':rideDetails.driverId,'complete':False,'cost':my_dist_1, 'expectedTime':rideDetails.expectedTime})
    return JsonResponse({'success':False,'driverId':"none",'complete':False,'cost':0, 'expectedTime':rideDetails.expectedTime})

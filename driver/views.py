...
def getAcceptedRides(request):
	...
	print(acceptList)
	return JsonResponse({ 'acceptList' : acceptList})
...
def rideHistory(request):
	driverDetails = ride.objects.filter(driverId=request.user.username)
	return render(request, 'rideHistory.html', {'d': driverDetails})
